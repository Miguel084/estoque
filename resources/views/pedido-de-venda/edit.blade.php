    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form action="{{ route('pedido-de-venda.update', $pedidoDeVenda->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="px-4 py-5 sm:p-6">
                        <label for="data_do_pedido" class="block font-medium text-sm text-gray-700">Pedido</label>
                        <input type="date" name="data_do_pedido" id="data_do_pedido" class="form-input rounded-md shadow-sm mt-1 block w-full" value="{{ $pedidoDeVenda->data_do_pedido->format('Y-m-d')  }}" />
                    ...
                    <div class="px-4 py-5 sm:p-6">
                        ...
                        <select name="produto_id" id="produto_id" class="form-select rounded-md shadow-sm mt-1 block w-full">
                            ...
                            @foreach($produtos as $produto)
                                <option value="{{ $produto->id }}" {{ $produto->id == $pedidoDeVenda->produto_id ? 'selected' : '' }}>{{ $produto->nome }}</option>
                            @endforeach
                        </select>
                        @error('produto_id')
                        <div class="text-red-500 mt-2 text-sm">
                    ...
                    <div class="px-4 py-5 sm:p-6">
                        ...
                        <select name="status" id="status" class="form-select rounded-md shadow-sm mt-1 block w-full">
                            ...
                            <option value="1" {{ $pedidoDeVenda->status == 1 ? 'selected' : '' }}>Aberto</option>
                            <option value="2" {{ $pedidoDeVenda->status == 2 ? 'selected' : '' }}>Fechado</option>
                        </select>
                        @error('status')
                        <div class="text-red-500 mt-2 text-sm">
                            {{ $message }}
